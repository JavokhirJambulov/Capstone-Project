...
while True:
    ...
    if len(faces) >= 1:
        ...
        # To check if the face of the last person and the current person the same
        ...
        # If the last person and the current person aren't same, we will continue with the following actions
        if last_person != person_name and person_name != 'Unknown':
            # Insert data into database
            c.execute("INSERT INTO face_captures (name, capture_time) VALUES (?, ?)", (person_name, current_time))
            conn.commit()
            # If the face changes, it will send it to company via Telegram for storage
