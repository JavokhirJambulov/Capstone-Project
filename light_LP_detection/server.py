...
def detect(car_m, lp_m, reader, frame):
    # import font to make the output more authentic
    fontpath = "SpoqaHanSansNeo-Light.ttf"
    font = ImageFont.truetype(fontpath, 20)

    # Create a copy of the input frame for drawing
    to_draw = frame.copy()

    # Detect cars in the frame using the car model
    results = car_m(frame)
    locs = results.xyxy[0]
    ...
    if len(locs) == 0:
        # Try detecting license plates in the frame
        result = lp_m(frame)
        if len(result) == 0:
            # No license plates detected
            result_text.append('No Car Plate Is Detected')
        else:
            # Process detected license plates
            for rslt in result.xyxy[0]:
                x2, y2, x3, y3 = [int(item.cpu().detach().numpy()) for item in rslt[:4]]
                try:
                    # Extract and resize the license plate region for recognition
                    extra_boxes = 0
                    ...
                    # Return the image if there's an error
            ...
            # Send recognized text via telegram messenger
    ...
    for idx, item in enumerate(locs):
        ...
        # Detect license plates within the detected car region
        result = lp_m(Image.fromarray(car_im))

        if len(result) == 0:
            result_text.append("Car is detected, but LP not recognized")
    ...
    # Return the processed image and recognized texts
    return cv2.resize(to_draw, (1280, 1280)), result_text
